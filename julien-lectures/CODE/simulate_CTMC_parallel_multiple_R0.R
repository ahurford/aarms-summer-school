# Example CTMC simulation of a simple SIS model, parallel version, with 
# multiple R_0 values.
# Here, we just want to know how many simulations see extinction, so we do 
# minimal post-processing

# BEWARE !!!
#
# With 100 sims and the number of values used, this can either be very lengthy 
# on a machine with low thread count or very expensive in RAM on a machine with 
# high thread count..
...
# Need a function that runs one simulation and returns a result. While we're at 
# it, we also return an interpolated solution
...
# Run the code (TRUE) or load the results from a file (FALSE). File must exist,
# of course.
if (TRUE) {
  ...
  for (I_0 in values_I_0) {
    ...
    # To process efficiently in parallel, we make a list with the different
    # parameter values we want to change, which will fed by parLapply 
    # to run_one_sim
    params_vary = list()
    # Vary R_0 and I_0
    i = 1
    ...
    stopCluster(cl)
    saveRDS(SIMS, file = sprintf("../OUTPUT/SIMS_I0_%d.Rds", I_0))
  }
} else {
  # If you have run quite a few sims and have saved the result, you may want to
  # skip the computation and instead just load said result.
  # Need to specify one value for I_0. (One set above or check the result files)
  I_0 = 2
  SIMS = readRDS(file = file = sprintf("../OUTPUT/SIMS_I0_%d.Rds", I_0))
}
#
...
# Plot
png(file = "../OUTPUT/extinctions_fct_R0.png",
